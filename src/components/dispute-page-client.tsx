...
export function DisputePageClient({ initialDispute, initialComments }: { initialDispute: Dispute, initialComments: Comment[] }) {
                ...
                <div className="space-y-4 rounded-lg border bg-muted/50 p-4">
                  ...
                  <div className="space-y-6">
                    {involvedPartyComments.map((comment) => (
                        <CommentCard key={comment.id} comment={comment} onDelete={handleDeleteDisputeComment} postId={dispute.id} postAuthorId={''} onReplySuccess={() => {}} />
                    ))}
                  </div>
                </div>
              )}
